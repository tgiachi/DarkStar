...
namespace DarkStar.Engine.MessageListeners
{
    ...
    {
        public TileSetMapMessageListener(ILogger<BaseNetworkMessageListener<TileSetMapRequestMessage>> logger, IDarkSunEngine engine) : base(logger, engine)
        {
        }
            ...
            DarkStarMessageType messageType, TileSetMapRequestMessage message)
        {
            if (Engine.PlayerService.GetSession(sessionId).IsLogged)
            {
                ...
                return SingleMessage(new TileSetMapResponseMessage(message.TileSetName, await TileSetHelper.GetTileSetMapAsync(message.TileSetName, Engine)));
            }
