...
namespace DarkSun.Engine.MessageListeners
{
    ...
    {
        public PlayerListMessageListener(ILogger<BaseNetworkMessageListener<PlayerListResponseMessage>> logger, IDarkSunEngine engine) : base(logger, engine)
        {
        }

        public override async Task<List<IDarkSunNetworkMessage>> OnMessageReceivedAsync(Guid sessionId, DarkSunMessageType messageType, PlayerListResponseMessage message)
        {
            var playerSession = Engine.PlayerService.GetSession(sessionId);
            if (playerSession.IsLogged)
            {
                return SingleMessage(await PlayerDataHelper.BuildPlayerListForPlayerAsync(Engine, playerSession.PlayerId));
            }
